...
- **明确蓝图触发条件**：多屏内容、多类型信息、复杂页面、形式与密度冲突、同时面向管理者和执行者时，先出结构方案。
- **新增蓝图输出要求**：必须说明读者、优先回答的问题、首屏内容、明细承接、手机端降级、格式不适配时的保底方式和默认方案。
- **新增不适配密度保底协议**：用户可以指定目标和外层形式，但 Agent 必须保护可读性、统一性和移动端适配；禁止靠缩小字号、减少留白、强制换行、增加颜色硬塞内容。
- **继续主 Skill 减法方向**：把机制写成通用决策流程，不把日历继续扩成主角。
